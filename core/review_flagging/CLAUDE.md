...
**`UnavailablePersistenceWriteGateway`'s gap is now closed too, in a separate pass once `core/persistence/generated/` shipped.** `gateways.py`'s new `GrpcPersistenceWriteGateway` is a real read-modify-write client against Persistence's now-real gRPC surface — `persistence.proto` still has no dedicated field-level `ApplyEdit` RPC (that part of the original gap is real and unchanged), so this gateway composes `GetReceipt`/`SaveReceipt` instead. Confirmed live against a real running `PersistenceGrpcServicer`: patching a first-class `Receipt` field (`total_amount`), patching a non-first-class field into `Receipt.fields` (`vendor_tin`), and a correct `receipt_not_found` error for an unknown receipt. `FlagStore`'s own shipped default stays `UnavailablePersistenceWriteGateway` deliberately — the same "don't flip a correctness-adjacent default silently" posture `core/accounting_sync/service.py`'s `NoOpFlagChecker` default takes.
