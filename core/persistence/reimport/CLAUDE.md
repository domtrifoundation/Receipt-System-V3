...
**`field_mapping.py` is not in the deep-dive's §2 package layout**, added with a reason. The export writes a deliberately lossier, human-facing view than the canonical row image Historian records — `transaction_date` date-only, `None` rendered as an empty cell — so the three sides of the diff are not directly comparable. Both canonical and baseline are projected into the workbook's own representation before comparison, and an applied cell is coerced back into the field's real type before it is written. Without that, a pure round trip with no user edit reported every dated receipt as user-changed and then put a `str` into a `datetime` column. An uncoercible cell is reported in `rejected_fields`, never written as-is and never silently dropped.
