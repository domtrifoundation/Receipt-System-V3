...
from core.account_guardian.privacy import deletion_request as dr
...
def test_a_clean_billing_state_enters_grace_period_immediately(db, audit_gateway, billing_gateway):
    ...
    result = run(dr.request_deletion(db, audit_gateway, billing_gateway, "user_a"))
...
def test_billing_hold_resumes_toward_grace_period_once_billing_clears(
    db, audit_gateway, billing_gateway, persistence_gateway
):
    billing_gateway.clear = False
...
def test_cancellation_correctly_fails_once_processing_has_started(
    db, audit_gateway, billing_gateway, persistence_gateway
):
...
def test_grace_period_elapsing_with_clean_billing_advances_all_the_way_to_complete(
    db, audit_gateway, billing_gateway, persistence_gateway
):
    request = run(dr.request_deletion(db, audit_gateway, billing_gateway, "user_a")).request
    future = utcnow() + timedelta(days=dr.DEFAULT_GRACE_PERIOD_DAYS + 1)
...
def test_advance_before_the_grace_period_elapses_is_a_no_op(
    db, audit_gateway, billing_gateway, persistence_gateway
):
    request = run(dr.request_deletion(db, audit_gateway, billing_gateway, "user_a")).request
...
def test_completed_deletion_is_recorded_as_a_distinct_audit_operation(
    db, audit_gateway, billing_gateway, persistence_gateway
):
    request = run(dr.request_deletion(db, audit_gateway, billing_gateway, "user_a")).request
    future = utcnow() + timedelta(days=dr.DEFAULT_GRACE_PERIOD_DAYS + 1)
